			...
			<div class="flex flex-col gap-3 pb-12">
				{#each nav as link}
					<a
						...
						class="font-semibold"
						on:click={closeSheet}>{link.label}</a
					>
					{#if link.subLinks}
						{#each link.subLinks as subLink}
							<a
								href={subLink.href}
								target={isExternalLink(subLink.href) ? '_blank' : ''}
								class="ml-4"
								on:click={closeSheet}>{subLink.label}</a
							>
							{#if subLink.subLinks}
								{#each subLink.subLinks as subSubLink}
									<a
										href={subSubLink.href}
										target={isExternalLink(subSubLink.href) ? '_blank' : ''}
